...
import {
    closest, select, isVisible, isDisabled, getCS, addClass, removeClass, hasClass, setAttr, removeAttr, getAttr, eventOn, eventOff,
} from './dom';
...
class ToolTip {
    ...
    updateConfig(config) {
        ...
        const updatedConfig = assign({}, this.constructor.Default, config);
    ...
    toggle(event) {
        ...
        } else if (hasClass(this.getTipElement(), ClassName.SHOW)) {
            this.leave(null);
        } else {
            this.enter(null);
        }
    ...
    getPopperConfig(placement, tip) {
        return {
            ...
            modifiers: {
                ...
                arrow: { element: '.arrow' },
                preventOverflow: { boundariesElement: this.$config.boundary, enabled: false },
            },
            onCreate: (data) => {
