...
const error = (format, ...args) => {
  let argIndex = 0;
  const message = `Error: ${format.replace(/%s/g, () => args[argIndex++])}`;
...
const createRootNode = rootNode => extend(rootNode || new Node(), {
  parent: null,
  children: [],
  state: {
    depth: -1,
...
class InfiniteTree extends events.EventEmitter {
    ...
    checkNode(node, checked, isLinked = true) {
      if (!ensureNodeInstance(node)) {
        return false;
      }
      if (typeof this.options.checkAble === 'function' && !this.options.checkAble(this.flattenNode(node), checked)) {
        return false;
      }
      ...
      const updateChildNodes = (parentNode) => {
        ...
        while (childNode) {
          ...
          childNode.state.checked = childNode.chkDisabled ? childNode.state.checked : parentNode.state.checked;
    ...
    // }, filterOptions);
    filterNodes(cb) {
      return this.nodes.filter(node => cb(node));
    }
    ...
    removeChildNodes(parentNode, options) {
      ...
      { // Update open nodes and lookup table
        this.state.openNodes = this.state.openNodes.filter(node => (removedNodes.indexOf(node) < 0) && node.state.open);
    ...
    scrollToNode(node) {
      ...
      let { firstChild } = this.contentElement;
      while (firstChild) {
