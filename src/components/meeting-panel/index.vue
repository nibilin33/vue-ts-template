  ...
  <div class="yl-cluster-meeting">
    <div class="border" :style="{padding:scrollBarWidth?'10px 0 10px 0':'10px 10px 10px 0'}">
      ...
      <el-row class="bg-white mg-top-20">
            ...
            <div class="pd-5">
              <el-checkbox-group v-model="checkedRoom">
                <div
                  ...
                  :key="index"
                  class="tr"
                  :class="{'border-solid-bottom':(isHiddenAble?!room.hide:true)}"
                >
                  <div
                    class="room-td"
                    :data-i="index"
                  ...
                  >
                    ...
                    <el-checkbox @change="singleChange($event,index)" :label="index">
       ...
       <div
        v-if="loading"
        class="meeting-panel__loading"
        :style="{height:height+'px'}">
       <loading
        custom-class="meeting-panel__loading--defined"></loading>
        </div>
      </el-row>
...
const getTimeList = function (granularity = 30) {
    const startTimestamp = new Date(`${getDate(new Date())}T00:00`).valueOf();
    ...
    methods: {
        ...
        scrollToTimePoint(date, point) {
            ...
            + $(`#${this.contentId}td0index${index}`).offset().left;
                    ...
                    $(this.$refs.scrollRight.$el).scrollLeft(scrollLeft - leftPosition - rightPosition);
                }
            }, 100);
        },
        ...
        handleSelect(item) {
            if (typeof $(`#room${item}`).offset() !== 'undefined') {
                ...
                $(this.$refs.scrollLeft.$el).scrollTop(rtop);
        ...
        destroyAll() {
            try {
                this.clusterize && this.clusterize.destroy();
                this.removeAllEvent();
        ...
        updateCheckedRoom(value) {
            console.log(this.checkedRoom, 'ddd');
        ...
        mouseOver(e, num, takeIndex) {
            if (typeof num !== 'undefined') {
                this.currentId = `popover${num}${$(e.target).data('index')}`;
        ...
        paintColor(start, end, row, isEmit = true) {
            ...
            if (isEmit) {
                ...
                this.$emit('choose-time', {
                    startTime:
            new Date(`${sdate}T00:00:00`).valueOf() + (start - 1) * eMinus * 60 * 1000,
                    endTime: new Date(`${edate}T00:00:00`).valueOf() + end * eMinus * 60 * 1000,
                });
            }
        },
